...
class Cache
{
    ...
    public static function path(string $cacheName)
    {
        ...
        //se nao existir o diretorio, cria o diretorio
    ...
    {
        /*aqui faz o cache funcionar     
        **filiemtime pega o tempo de criacao do arquivo 
        **callback é o retorno dos dados do banco de dados
        */
        $cachePath = self::path($cacheName);

        if(!file_exists($cachePath) || strtotime("+{$expiration} minutes", filemtime($cachePath)) < strtotime('now')){
            // var_dump('guardou no cache e retornou o valor do callback');
